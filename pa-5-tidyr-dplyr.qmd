...
military <- read_xlsx("data/gov_spending_per_capita.xlsx",  # reads file path of data
                      sheet = "Share of Govt. spending",    # selects tab on excel sheet
                      skip  = 7,                            # skipping a number of rows
                      n_max = 191,                          # reads a maximum number of rows
                      na = c("xxx", ". ."))                  # reads as na's 
```
...
military_EEurope <- military_new |>
  filter(Country == eastern_europe) 

ggplot(data = military_EEurope, aes(x = Country, y = spending)) +
  geom_boxplot() +
  coord_flip() +
  labs(title = " Eastern Eripe Military Spending",
       y = "Spending Proportion of GDP",
       x = "Country") 
